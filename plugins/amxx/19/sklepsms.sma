...
public plugin_init()
{
	...
	g_pNetworkID = register_cvar("shop_netid", "1");
...
CheckLicense()
{
	...
	formatex(qCommand, charsmax(qCommand), "SELECT UNIX_TIMESTAMP(`license_expire`) AS `expire`, `license_expire` FROM \
	         `networks` WHERE `id` = %d AND `active` = 1;", get_pcvar_num(g_pNetworkID));
...
public CheckLicenseHandler(FailState, Handle:Query, Error[], Errorcode)
{
	...
	{
		log_amx("[SklepSMS] Licencja #%d nie jest aktywna", get_pcvar_num(g_pNetworkID));
	...
	serverData[NETWORK_ID] = get_pcvar_num(g_pNetworkID);
