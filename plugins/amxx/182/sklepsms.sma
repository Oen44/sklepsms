...
public ShopMenu(id) {
	...
	menu_addblank(menu);
	menu_addtext(menu, "\dSklep by\y SklepSMS.pl");
	...
	if (socket_change(g_iSocket[id]))
	{
		...
		for(new i = 0; i < count; i++)
		{
			...
			if (equali(line_variable, "success"))
			{
				...
				{
					ColorChat(id, RED, "^1[^3SklepSMS^1] Usluga dodana poprawnie!");
					if(g_iAuthType[id] == TYPE_NICK) ColorChat(id, RED, "^1[^3SklepSMS^1] Wpisz w konsoli^x04 setinfo _pw ^"%s^"", g_szPassword[id]);
					ColorChat(id, RED, "^1[^3SklepSMS^1] Usluga zostanie aktywna po ponownym polaczeniu z serwerem");
					LoadAccess();
				}
				break;
			}
...
public AdminCodeMenuHandler(id, menu, item)
{
	...
	{
		ColorChat(id, RED, "^1[^3SklepSMS^1] Prosze nie spamowac!");
		menu_destroy(menu);
		return PLUGIN_HANDLED;
	}
	...
	if (socket_change(g_iSocket[id]))
	{
		...
		for(new i = 0; i < count; i++)
		{
			...
			if (equali(line_variable, "success"))
			{
				...
				ColorChat(id, RED, "^1[^3SklepSMS^1] Wygenerowany kod SMS:^4 %s", line_value);
			}
		}
	}
}
...
public cmd_SmsCode(id) {
	...
	ColorChat(id, RED,"^x01[^x03SklepSMS^x01] Sprawdzam Kod SMS...");
}
...
CheckAccess(id) {
	...
	if(TrieKeyExists(g_accessTrie, userip)) {
		...
		if(equali(md5Password, access[ACCESS_PASS]))
			return set_user_flags(id, read_flags(access[ACCESS_FLAGS]));
		...
		for(new i = 0; i < count; i++) {
			...
			if (equali(line_variable, "success")) {
				...
				if(returnValue < serviceValue) {
					ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Wartosc wpisanego kodu wynosi^x03 %.2f^x01 a uslugi^x03 %.2f", returnValue, serviceValue);
					ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Usluga nie bedzie dodana a kod zostal wykorzystany, prosimy nie oszukiwac.");
				}
				else {
					new ret;
					switch(service[SERVICE_TYPE]) {
						...
						case SERVICE_TYPE_FLAGS: {
							g_fServiceBought = CreateOneForward(service[SERVICE_PlUGIN], "shop_service_bought", FP_CELL, FP_CELL);
							ExecuteForward(g_fServiceBought, ret, id, price[PRICE_AMOUNT]);
							ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Usluga zakupiona poprawnie!");
							if(g_iAuthType[id] == TYPE_NICK) ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Wpisz w konsoli^x04 setinfo _pw ^"%s^"", g_szPassword[id]);
							ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Usluga zostanie aktywna po ponownym polaczeniu z serwerem");
							LoadAccess();
						}
					}
					DestroyForward(g_fServiceBought);
				...
				ColorChat(id, RED, "^x01[^x03SklepSMS^x01] Blad:^x03 %s", line_value);
				break;
			}
		}
